    ...
    for i in range(30):
        if i <= 9:
            task1 = BashOperator(
                ...
                doc_md=dedent(
                    """
                    #BashOperator *выполняет* произвольную команду*, 
                    так или иначе **использующую** переменную цикла
                    (например, можете указать 'echo {i}').
                    """
                )
            )
        else:
            task2 = PythonOperator(
                ...
                doc_md=dedent(
                    """
                    #PythonOperator *выполняет* функцию, которая задействует переменную из цикла.
                    Передается переменная через **op_kwargs**. Функция печатает
