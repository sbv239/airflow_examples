...
with DAG(
        ...
        catchup=False,
        tags=['SimJul_7'],  # теги, способ помечать даги
) as dag:
    def print_task_number(task_number, ts, run_id):
        return (f'task number is: {task_number}, i_={i}, ts = {ts}, run_id = {run_id}')  # Функция должна печатать "task number is: {task_number})


    for i in range(30): #10 задач сделайте типа BashOperatort
        if i<10:
            t1=BashOperator(
                task_id=f'hw_7_j-miller-20_{i}',  # id, будет отображаться в интерфейсе
                bash_command=f'echo {i} ',  # можете указать f"echo {i}"
                dag = dag
            )
        else:
            t2 = PythonOperator(
            task_id=f'hw_7_j-miller-20_{i}',#id, как у всех операторов
