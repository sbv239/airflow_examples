...
with DAG(
        ...
        catchup=False,
        tags=['SimJul_6'],  # теги, способ помечать даги
) as dag:

    for i in range(30): #10 задач сделайте типа BashOperatort
        if i<10:
            t1=BashOperator(
                task_id=f'hw_6_j-miller-20_{i}',  # id, будет отображаться в интерфейсе
                bash_command=f'echo $NUMBER ',  # # передаем переменную окружения
                env={'NUMBER': str(i)},  # задаем значение переменной окружения
                dag = dag
            )
        else:
